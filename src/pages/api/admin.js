...
export const adminApi = {
  ...
  listUsers: (skip = 0, limit = 100) =>
    client.get('/api/admin/users', { params: { skip, limit } }),

  // GET /api/admin/users/{user_id}
  // Response: { id, username, email, full_name, is_admin, is_active, created_at }
  getUser: (userId) => client.get(`/api/admin/users/${userId}`),

  // DELETE /api/admin/users/{user_id}
  // Response: string (success message)
  deleteUser: (userId) => client.delete(`/api/admin/users/${userId}`),

  // POST /api/admin/users/{user_id}/make-admin
  // Response: string (success message)
  makeAdmin: (userId) => client.post(`/api/admin/users/${userId}/make-admin`),

  // POST /api/admin/users/{user_id}/deactivate
  // Response: string (success message)
  deactivateUser: (userId) => client.post(`/api/admin/users/${userId}/deactivate`),

  // POST /api/admin/users/{user_id}/activate
  // Response: string (success message)
  activateUser: (userId) => client.post(`/api/admin/users/${userId}/activate`),

  // GET /api/admin/resumes?skip=&limit=
  listResumes: (skip = 0, limit = 100) =>
    client.get('/api/admin/resumes', { params: { skip, limit } }),

  // GET /api/admin/searches?skip=&limit=
  listSearches: (skip = 0, limit = 100) =>
    client.get('/api/admin/searches', { params: { skip, limit } }),

  // GET /api/admin/applications?skip=&limit=
  listApplications: (skip = 0, limit = 100) =>
    client.get('/api/admin/applications', { params: { skip, limit } }),
}